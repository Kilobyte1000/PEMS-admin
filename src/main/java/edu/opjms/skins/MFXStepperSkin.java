...
import edu.opjms.controls.MFXStepperToggle.MFXStepperToggleEvent;
import edu.opjms.global.AnimationKt;
...
public class MFXStepperSkin extends SkinBase<MFXStepper> {
    ...
    private void setListeners() {
        ...
        stepper.getStepperToggles().addListener((InvalidationListener) invalidated -> {
            ...
            pause.setOnFinished(actionEvent -> stepper.requestLayout());
            pause.play();
        });
    ...
    private void updateProgressBar(double width) {
        ...
        KeyFrame kf = new KeyFrame(Duration.millis(stepper.getAnimationDuration()), new KeyValue(bar.widthProperty(), width, AnimationKt.MFX_INTERPOLATOR_V2));
